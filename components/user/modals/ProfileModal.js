...
const ProfileModal = (props) => {
  ...
  return (
    <Modal
      ...
      centered
      size="xl"
      onHide={props.hideProfileModal}
      className="profileModal pl-0"
    >
      <Modal.Body className=" text-left mx-0">
        <div className="postsHeadingProfile mt-0">Friends List</div>

        <div
          className={` ${props.myRequests.count ? "d-md-block" : "d-md-none"}`}
        >
          <div className="friendsRequestsMainCont">
            {props.myRequests.requests && props.myRequests.requests.length
              ? props.myRequests.requests.map((requester, index) => {
                  return (
                    index < props.maxRequestsToshow && (
                      <Request
                        history={history}
                        dispatch={dispatch}
                        requester={requester}
            ...
            {props.myFriends.isError ? (
              ...
              props.myFriends.data.friends.length > 0 && (
                <>
                  <div
                    className="profileFriendsCont profileFriendsContModal w-100"
                    id="friendList2"
                  >
                    <InfiniteScroll
      ...
      <Modal.Footer className="pt-0 justify-content-center"></Modal.Footer>
    </Modal>
  );
};
